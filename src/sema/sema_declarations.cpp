    ...
    if (cls->methods) {
        ...
        for (auto& m : cls->methods->methods) {
            ...
            if (!func->isDestructor) {
                // destructors never overload and are never duplicate-checked here —
                // they already have their own uniqueness check (destructorCount) above.
                // Without this guard, a destructor and a no-arg constructor (both named
                // after the class, both 0 parameters) collide in the same overload bucket.
